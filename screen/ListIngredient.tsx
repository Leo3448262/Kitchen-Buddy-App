...
export default function IngredientListScreen() {
    ...
    <VStack className="p-4">
      {ingredients.map((item, index) => (
        ...
        <Card  className="mb-4 p-4 shadow-md rounded-lg bg-white">
                  <Button key={index}className="ml-auto" onPress={() => {
  ...
  >
    ...
    {isRipenessExpired(item.ripeness, item.ripenessUpdatedAt) && " (Expired)"}
  </Text>
) : null}

         <Text>
            Expiry: {item.selectedDate ? new Date(item.selectedDate).toDateString() : `${item.estimate} days from now`}
          </Text>
        </Card>
