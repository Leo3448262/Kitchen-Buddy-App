...
export default function EditIngredientScreen() {
  const {selectedIngredient, setSelectedIngredient, updateIngredient} =
    useIngredientContext();
  ...
  useEffect(() => {
    if (selectedIngredient) {
      ...
      setRipenessUpdatedAt(
        selectedIngredient.ripenessUpdatedAt
          ? new Date(selectedIngredient.ripenessUpdatedAt)
          : null,
      );
      setIsFrozenChecked(selectedIngredient.isFrozen ?? false);
      setIsOpenedChecked(selectedIngredient.isOpened ?? false);
      setOpenedAt(
        selectedIngredient.openedAt
          ? new Date(selectedIngredient.openedAt)
          : null,
      );
    }
  }, [selectedIngredient]);
      ...
      <VStack className="items-center justify-center h-auto">
        <FormControl className="p-5 border rounded-lg border-outline-300">
          <VStack space="xl">
            ...
            <VStack>
              <Select selectedValue={location} onValueChange={setLocation}>
                <SelectTrigger>
                  <SelectInput
            ...
            <VStack>
              <Select selectedValue={confection} onValueChange={setConfection}>
                <SelectTrigger>
                  <SelectInput
                ...
                <SelectPortal>
                  ...
                  <SelectContent>
                    ...
                    {confections.map(confection => (
                      <SelectItem
                        key={confection.value}
                        label={confection.label}
                        value={confection.value}
                      />
                    ))}
                  </SelectContent>
                </SelectPortal>
